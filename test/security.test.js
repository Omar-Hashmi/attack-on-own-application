...
test('attempt 5: a forged random session cookie is not accepted', async () => {
  // Request: GET /api/appointments/apt-alice
  // Cookie: session=admin-user-alice (a guessed, unissued token)
  const response = await api('/api/appointments/apt-alice', {
    headers: {
      cookie: 'session=admin-user-alice',
    },
  });

  // Expected response: 401 { "error": "Authentication required" }
